...
- **Comprehensive Case-Insensitive Dependency Classification**: Extended case-insensitive matching (`in~` and `has`) across all dependency types in `BuildDependenciesSummary` and `BuildLatencyBreakdown` (supporting Redis, Memcached, HTTP, gRPC, and Webservice alongside SQL), preventing case mismatch drops across heterogeneous SDK telemetry instrumentation.
...
- **Standardized Column Aliases & Clear PascalCase Naming**: Unified column naming conventions across all KQL queries with zero-cost projection aliases:
  - `Endpoint` across `endpoints` and `n-plus-one` (replacing ambiguous system `name`).
  - `Type`, `Target`, and `Dependency` across `dependencies` telemetry.
  - `Deprecation` for framework deprecation warning summaries.
  - `Type` and `Message` in exception summaries.
  - Aligned headers for seamless direct inspection in Azure Portal and CLI table outputs.
...
- **Multi-Stack, Production-Ready Deprecations Engine**: Upgraded `azlens inspect deprecations` KQL query with token-indexed (`has_any`) filtering to capture deprecations across the 5 core target stacks (Python/Django `RemovedInDjango`/`DeprecationWarning`, Ruby on Rails `DEPRECATION WARNING`/`ActiveSupport::DeprecationException`, Node.js `[DEP` warnings, Java/Spring deprecations, and .NET/C# `CS0618`/`CS0612`/`Obsolete`) while filtering out SQL statements and normalizing file line numbers, process IDs (`(node:<pid>)`), and memory pointers (`0x<hex>`) for high-performance grouping and zero query performance loss.
...
- **Removed Legacy Configuration Parameters & Fallbacks**: Removed legacy `role` (replaced by `role_name`), legacy `ExitCodeQuality` process exit code alias, and obsolete configuration parameters, eliminating all legacy compatibility branches in configuration schemas and validation.
